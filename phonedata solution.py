...
phone = []
with open("myphonedata.csv") as f:
    phone = f.read()
...
plt.bar(height = frequencies[1:14],left = list(range(1,14)),color='rgbkymc');plt.xticks(list(range(1,14),),items[1:14]);plt.xlabel("items")
plt.ylabel("Count");plt.xlabel("Items")

# Creating Data Frame for the transactions data 

phone_series  = pd.DataFrame(pd.Series(phone_list))
phone_series = phone_series.iloc[:166,:] # removing the last empty transaction
